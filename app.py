...
# Ensure CSV files exist and basic structure
...
# --------------------------------
if st.button("Logout"):
    for k in ["logged_in","user_type","employee_id","username"]:
...
if st.session_state.user_type=="employee":
    ...
    if st.button("Submit Check"):
        if not oil_photo:
            ...
            entry={
                "submission_id":sid,"Employee":emp_row.Employee,"Vehicle":assigned_vehicle,"Date":date_str,
            ...
            mdf=pd.concat([mdf,pd.DataFrame([{"submission_id":sid,"Employee":emp_row.Employee,"Vehicle":assigned_vehicle,"Date":date_str,"Mileage":mileage,"Mileage_Comments":mileage_comments}])],ignore_index=True)
...
with tabs[0]:
    ...
    # Recent notes
    df_sub=load_data(DATA_FILE); df_sub.Date=pd.to_datetime(df_sub.Date)
    vs=df_sub[df_sub.Vehicle==sel].sort_values("Date",ascending=False)
    with st.container():
        st.write("### Recent Notes (2 Months)")
        for _,r in vs.iterrows():
            if r.Notes and r.Date>=datetime.now()-timedelta(days=60):
        ...
        for _,r in vs.head(5).iterrows():
            with st.expander(f"{r.Employee} - {r.Date.date()}"):
                ...
                if st.button("Delete",key=r.submission_id):
                    dfa=load_data(DATA_FILE); dfa=dfa[dfa.submission_id!=r.submission_id]; dfa.to_csv(DATA_FILE,index=False)
                    mfa=load_data(MILEAGE_FILE); mfa=mfa[mfa.submission_id!=r.submission_id]; mfa.to_csv(MILEAGE_FILE,index=False)
                    st.experimental_rerun()
...
with tabs[1]:
    ...
    if not df_sub.empty:
        ...
        sel_w=st.selectbox("Select Week",weeks,index=weeks.index(ws) if ws in weeks else 0,format_func=lambda x:x.date())
        wdf=df_sub[(df_sub.Date>=sel_w)&(df_sub.Date<sel_w+timedelta(days=7))]
        rows=[]
        for _,e in emp_df.iterrows():
            subset=wdf[wdf.Employee==e.Employee]
            rows.append({"Employee":e.Employee,"Assigned Vehicle":e.Assigned_Vehicle,"Status":"Submitted" if not subset.empty else "Not Submitted","Date Submitted":subset.iloc[0].Date.date() if not subset.empty else "N/A"})
